...
两者之间的主要差别是 IP 地址所用的字节数，目前通用的是 IPV4 , IPV6 正在普及。

IPV4 标准的 4 字节 IP 地址分为网络地址和主机（指计算机）地址，且分为 A、B、C、D、E 等类型。

![IPv4地址族](images/IPv4地址族.png)

网络地址(网络ID)是为区分网络而设置的一部分IP地址。假设向www.semi.com公司传输数据，该公司内部构建了局域网，把所有计算机连接起来。首先应向semi.com网络传输数据，并发一开始就浏览4字节地址，进而找到目标主机，而是仅浏览4字节IP地址的网络地址，先把数据传到SEMI.COM的网络。SEMI.COM的网络(构成网络的路由器)接收带数据后，浏览传输的主机地址(主机ID)并就昂数据传给目标计算机。

数据传输过程：

![基于IP地址的数据传输过程](images/基于IP地址的数据传输过程.png)
...
![大端序字节表示](images/大端序字节表示.png)
![小端序字节表示](images/小端序字节表示.png)

两台字节序不同的计算机在数据传递的过程中可能出现的问题：

![字节序问题](images/字节序问题.png)
...
[endian_conv.c](Linux/endian_conv.c)
...
#### 函数 htons、htonl 在Windows中的使用

调用htons函数和htonl函数的示例：
[endian_conv_win](Windows/endian_conv_win.c)

#### inet_addr、inetntoa在Windows中的使用

调用函数inet_addr、inetntoa的示例
[inet_adrconv_win](Windows/inet_adrconv_win.c)
...
Winsock2中增加的2个转换函数，在功能上与inet_ntoa和inet_addr完全相同，但优点在于支持多种协议，在IPv4 和 IPv6均可适用。
缺点是依赖于特定平台，不能跨平台。
...
使用WSAStringToAddress 和 WSAAddressToString 的示例
[conv_addr_win](Windows/conv_addr_win.c)
...
#### 10怎样表示回送地址？其含义是什么？如果向会送地址处传输数据将会发生什么情况？
